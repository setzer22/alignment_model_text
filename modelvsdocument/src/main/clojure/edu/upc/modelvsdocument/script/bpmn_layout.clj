(comment (ns edu.upc.modelvsdocument.script.bpmn_layout
         ...
         (defn autoLayoutModel
           ...
           (let [model (bpmn/build-model (bpmn/read-model model-path))
                 ...
                 __ (.execute layout)
                 task-ids (bpmn/all-element-ids model)
             ...
             (doseq [process (:processes model)
               ...
               (set-pool-dimensions! model pool-id bounding-box {:top 30 :bottom 30 :left 30 :right 30}))
             ;; Lane positions
             (doseq [lane (specter/select [:processes specter/ALL :lanes specter/ALL] model)
                     :let [ids (.getFlowReferences lane)
                           bounding-box (compute-bounding-box model ids)
                           lane-id (.getId lane)]]
               (set-pool-dimensions! model lane-id bounding-box {:top 30 :bottom 30 :left 0 :right 30}))
             ;; Final repositioning
